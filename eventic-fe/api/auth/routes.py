...
from db.db_connect import get_db_connection, get_test_connection

auth_bp = Blueprint("auth", __name__)
bcrypt = Bcrypt()

@auth_bp.route("/mytest", methods=["GET"])
def mytest():
    try:
        with get_test_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute("SELECT id, user_name, email, phone, date_of_birth, sec FROM users")
